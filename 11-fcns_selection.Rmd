...
**`nodes`** an optional vector of node IDs for filtering list of nodes present in the graph.
...
**`expressions`** one or more expressions for filtering of nodes by degree values. Use a combination of a degree type (`deg` for total degree, `indeg` for indegree, and `outdeg` for outdegree) with a comparison operator and values for comparison (e.g., use \code{"deg >= 2"} to select nodes with a degree greater than or equal to 2).
...
Select edges in a graph object of class `dgr_graph` using edge ID values. **`graph`** a graph object of class `dgr_graph`.
