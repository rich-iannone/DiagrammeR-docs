...
For certain graph diagrams there will be many nodes and edges, and, it may not be practical to add nodes/edges a few at a time. Let's use the `create_node_df()` function to specify a collection of nodes and contain them in a data frame (a *node data frame*, or *ndf*). Immediately after that, inspect the node data frame.
