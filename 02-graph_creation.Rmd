...
Adding nodes does not add edges. We can check that there are no edges in the graph by using the `edge_count()` function. A graph with no edges will return `0`.
...
We now have this simple and small graph. We can view it in the RStudio Viewer by using the `render_graph()` function. You probably do not want to assign the graph to an object when calling `render_graph()` (as risk mistakenly overwriting a graph you've previously made). We are purely using this function for its side effect, which is viewing. We can view the graph using the **Graphviz** renderer:
...
There are some shortcuts/variations for doing the same thing (there are often numerous ways to transform graphs). You could simply call `delete_node()` twice, for instance, to get an empty graph. This is because removing a node with edges attached will automatically remove those edges. 
...
Let's view the graph using the `render_graph()` function.
